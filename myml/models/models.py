...
class MLRegressor:
    ...
    def kfold_crossvalidation(self, n_splits, n_repeats=1, style="KFold", random_state=None, find_outfliers=False):
        ...
        kfold_key_keys = ["imodel", "score",
                          "imax_rerror", "comp_max_rerror", "max_rerror",
                          "imax_error", "comp_max_error", "max_error"]
        ...
        for key in self.keys:
            ...
            for train, test in rkf.split(X, y4split):
                ...
                if find_outfliers:
                    ...
                    max_rerror = np.max(np.abs(errors / y_test))
                    imax = np.argmax(np.abs(errors))
                    imax_error = test[imax]
                    irmax = np.argmax(np.abs(errors / y_test))
                    imax_rerror = test[irmax]
                    comp_max_error = self.data.gooddf.iloc[imax_error]["Composition"]
                    comp_max_rerror = self.data.gooddf.iloc[imax_rerror]["Composition"]
                    if imodel % 10 == 0:
                        print(f"imax_rerror:{imax_rerror} imax_error:{imax_error}")
                        print(f"max_rerror:{max_rerror} max_error:{max_error}")
                        print(f"target_rerror:{y_test[irmax]} target_error:{y_test[imax]}")
                        print(f"pred_rerror:{preds[irmax]} pred_error:{preds[imax]}")
                        print(f"comp_max_rerror:{comp_max_rerror} comp_max_error:{comp_max_error}")
                        print(f"--- {imodel} ---")

                thisdict = {"imodel": imodel, "score": thisscore,
                            "imax_rerror": imax_rerror, "comp_max_rerror": comp_max_rerror, "max_rerror": max_rerror,
                            "imax_error": imax_rerror, "comp_max_error": comp_max_error, "max_error": max_error}
